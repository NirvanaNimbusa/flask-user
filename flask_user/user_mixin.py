...
class UserMixin(FlaskLoginUserMixin):
    ...
    def get_id(self):
        ...
        user_id = self.id
        password_ends_with = '' if user_manager.USER_ENABLE_AUTH0 else self.password[-8:] if self.password else ''
    ...
    def get_user_by_token(cls, token, expiration_in_seconds=None):
        ...
        if data_items:
            ...
            user_password = '' if user_manager.USER_ENABLE_AUTH0 else user.password[-8:] if user.password else ''
